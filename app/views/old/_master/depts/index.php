            ...
            <form id="myForm">
                <div class="modal-body">
                    ...
                    <div id="formDataWrapper">
                        <input type="hidden" id="input-dept-id" name="id">
                        <div class="row">
                            <div class="col-md">
                                <label for="input-dept-name" class="form-label">Dept. Name</label>
                                <input type="text" class="form-control" id="input-dept-name" name="dept_name" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    let loadData = (...args) => {
        ...
        request.open('GET', '<?= BASEURL ?>/depts/getAll');
            ...
            if(request.readyState == 4 && request.status == 200) {
                ...
                results.forEach((result) => {
                    ...
                    let btnDelete = document.createElement('button');
                        ...
                        btnDelete.setAttribute('data-name', result.dept_name);
                        btnDelete.innerHTML = '<i class="bi bi-trash3-fill"></i>';

                    td1.innerHTML = no++;
                    td2.innerHTML = result.dept_name;
                    td3.append(btnEdit);
                    td3.append(' ');
    ...
    let deptName = document.getElementById('input-dept-name');
    ...
    const validate = () => {
        let formData = 'dept_name=' + deptName.value;
        let request = new XMLHttpRequest();
        request.open('POST', '<?= BASEURL ?>/depts/validate');
        ...
        request.onreadystatechange = () => {
            if(request.readyState == 4 && request.status ==  200) {
                ...
                if(results) {
                    ...
                    alertMsgModal.innerHTML = '<b>' + results.dept_name + '</b> sudah terdaftar !';
                    alertModalResponse();
                } else {
    ...
    }; deptName.addEventListener('keyup', validate);
    ...
    const createData = () => {
        let formData = 'dept_name=' + deptName.value;
        let request = new XMLHttpRequest();
        request.open('POST', '<?= BASEURL ?>/depts/store');
        request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        request.onreadystatechange = () => {
            if(request.readyState == 4 && request.status == 200) {
                let results = JSON.parse(request.responseText);
    ...
    const getButtons = (...args) => {
        ...
        for(let i = 0; i < btnEdit.length; i ++) {
            btnEdit[i].addEventListener('click', () => {
                ...
                let request = new XMLHttpRequest();
                request.open('POST', '<?= BASEURL ?>/depts/getDataById');
                request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                request.onreadystatechange = () => {
                    if(request.readyState == 4 && request.status == 200) {
                        let results = JSON.parse(request.responseText);
                        deptId.value = results.id;
                        deptName.value = results.dept_name;
                    }
                };
                request.send(formData);
            });
    ...
    // proses update data
    const updateData = () => {
        let formData = 'id=' + deptId.value + '&dept_name=' + deptName.value;
        let request = new XMLHttpRequest();
        request.open('POST', '<?= BASEURL ?>/depts/update');
        request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        request.onreadystatechange = () => {
            if(request.readyState == 4 && request.status == 200) {
                let results = JSON.parse(request.responseText);
    ...
    const deleteData = () => {
        ...
        let request = new XMLHttpRequest();
            request.open('POST', '<?= BASEURL ?>/depts/destroy');
            request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            request.onreadystatechange = () => {
                if(request.readyState == 4 && request.status == 200) {
                    let results = JSON.parse(request.responseText);
                    dataName = results.dept_name;
                    alert.setAttribute('class', results.alertClass);
                    alertMsg.innerHTML = results.alertMsg;
                    alertResponse();
                    formClear();
    ...
    let searchData = () => {
        ...
        let request = new XMLHttpRequest();
        request.open('POST', '<?= BASEURL ?>/depts/search');
        request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        request.onreadystatechange = () => {
            if(request.readyState == 4 && request.status == 200) {
                let results = JSON.parse(request.responseText);
