  ...
  <div class="col-12 col-md-12" id="colDataTable">
    <div class="card">
      <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link active" href="<?= BASEURL . '/event_cash' ?>"><i class="bi bi-download"></i> Pemasukan</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="<?= BASEURL . '/event_cash/pengeluaran' ?>"><i class="bi bi-upload"></i> Pengeluaran</a>
        </li>
      </ul>
      ...
      <div class="card-body table-responsive mt-3">
        <div class="row mb-3">
          <div class="col">
            <a href="<?= BASEURL . '/event_cash/catat_pemasukan' ?>" class="btn btn-success btn-sm"><i class="bi bi-plus-circle"></i> Pemasukan</a>
        ...
        <table class="table table-striped display caption-top" id="myTable">
          <caption id="captionTable"><span class="text-muted">Pemasukan Semua Acara</span></caption>
          <thead>
            <tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="dateRangeReportLabel">Laporan Kas Masuk Acara</h1>
      ...
      <div class="modal-body">
        <form id="formReport" method="POST">
          ...
          <script>
            $('#ref_event').on('change', () => {
              $('#formReport').attr('action', '<?= BASEURL . '/event_cash/laporan_pemasukan/' ?>' + $('#ref_event').val());
            });
          </script>
...
<script>
  function confirmDelete(id) {
    if(confirm(`Yakin hapus data ini ?`)) {
      $.ajax({
        url: '<?= BASEURL . "/event_cash/pemasukan_delete/" ?>' + id,
        type: 'POST',
        data: 'id=' + id,
        success: function(res) {
          if(res == 'success') {
            Swal.fire({
              icon: 'success',
              title: 'Berhasil menghapus data pemasukan',
            ...
            Swal.fire({
              icon: 'error',
              title: 'Gagal menghapus data pemasukan !',
              text: res,
              showConfirmButton: true,
            })
          }
  ...
  $(function() {
    $('#myTable').dataTable({
      ...
      'serverSide': true,
      'ajax': {
        'url': '<?= BASEURL . "/event_cash/kasMasukAjax" ?>',
        'dataType': 'json',
        'type': 'POST'
      },
      'columns': [
