    ...
    <div class="card">
      ...
      <div class="card-body">
        <form id="formEdit">
          ...
          <label for="qty" class="form-label" id="labelRupiah">Nominal Zakat Standar</label>
          <div class="input-group mb-3">
            <span class="input-group-text" id="labelRupiah">Rp.</span>
            <input type="number" class="form-control" id="qty_in" name="qty_in" autocomplete="off" value="<?= str_replace('.00', '',$data['muzakki'][0]['qty_in']) ?>" required>
          </div>
...
<script>
  ...
  $('#formEdit').on('submit', (e) => {
    ...
    $.ajax({
      url: '<?= BASEURL . "/zakat_fitrah/uang_masuk_update" ?>',
      type: 'POST',
      data: formData,
