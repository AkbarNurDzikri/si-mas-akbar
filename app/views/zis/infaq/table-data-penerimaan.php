...
<?php foreach($data['infaq_masuk'] as $maal) : ?>
  ...
  <div class="col-12 col-md-12" id="colDataTable">
    <div class="card">
      <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link active" href="<?= BASEURL . '/infaq' ?>"><i class="bi bi-download"></i> Penerimaan</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="<?= BASEURL . '/infaq/pengeluaran' ?>"><i class="bi bi-upload"></i> Pengeluaran</a>
        </li>
      </ul>
      ...
      <div class="card-body table-responsive mt-3">
        <div class="row mb-3">
          <div class="col">
            <a href="<?= BASEURL . '/infaq/catat_penerimaan' ?>" class="btn btn-success btn-sm"><i class="bi bi-plus-circle"></i> Penerimaan</a>
        ...
        <table class="table table-striped display caption-top" id="myTable">
          ...
          <thead>
            <tr>
              ...
              <th class="text-center align-middle">Nama Donatur</th>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="dateRangeReportLabel">Laporan Penerimaan Infaq</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="<?= BASEURL . '/infaq/laporan_penerimaan' ?>" method="POST">
...
<script>
  ...
  function confirmDelete(id, name) {
    if(confirm(`Yakin hapus data ${name} ?`)) {
      $.ajax({
        url: '<?= BASEURL . "/infaq/penerimaan_delete/" ?>' + id,
        type: 'POST',
        data: 'id=' + id,
  ...
  $(function() {
    $('#myTable').dataTable({
      ...
      'serverSide': true,
      'ajax': {
        'url': '<?= BASEURL . "/infaq/infaqMasukAjax" ?>',
        'dataType': 'json',
        'type': 'POST'
      },
      'columns': [
